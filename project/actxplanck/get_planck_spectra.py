...
for freq in freqs:
    ...
    for hm, map in zip(splits, maps):
        ...
        badpix = (cov_map.data == 0)
        window_t[badpix] = 0
        window_pol[badpix] = 0
        # badpix = (cov_map.data == hp.pixelfunc.UNSEEN) ## This is a bug. Should be == 0
        # for i in range(3): # I think it's better to make sure it's masked than to null
